plot_regional <- function(files, r_num, years = c(0:9)) {
    
    # Set up for graphs
    
    par(mfrow = c(1, 1), mar = c(5.5, 5, 4, 5.5))
    month_labels = c("July", "August", "September", "October", "November", "December",
    ...
    for (i in years) {
        ...
        title <- paste("Region", r_num,  "Comparison of ILINet and GFT Data; Year =", year_labels[i + 1], ", Corr =", formated_corr)
