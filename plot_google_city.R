...
for (i in which_cities){
  ...
  plot(city[a:b,i], type = "l", col = Colors[c],ylim = g_range, axes = FALSE, ann = FALSE)
  c = c+1
  par(new = T)
}
...
title(main = "Cities", col.main="black", font.main=4)
        ...
        legend("topright",names(city)[which_cities-2],lwd = c(2.5,2.5),col = Colors,cex=0.4, ncol=2)
       par(new = FALSE)
}
}
