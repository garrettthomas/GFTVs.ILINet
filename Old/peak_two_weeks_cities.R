...
find_peak_two_week_region <- function(which_years = c(0:9)) {
    ...
    for (j in which_years) {
        ...
        for (k in 2:98) {
            ...
            for (i in a:b-1) {
                ...
                if (!is.null(cities[i, k]) && !is.null(cities[i+1, k]) && !is.na(cities[i, k]) && !is.na(cities[i+1, k])
                    && (as.numeric(cities[i, k]) + as.numeric(cities[i+1, k])) > point[1]) {
