...
find_peak_three_week_cities <- function(which_years = c(0:9)) {
        ...
        for (k in 2:98) {
            ...
            for (i in a:b-2) {
                ...
                if (!is.null(cities[i, k]) && !is.null(cities[i+1, k]) && !is.null(cities[i+2, k])
                    ...
                    && (as.numeric(cities[i, k]) + as.numeric(cities[i+1, k]) + as.numeric(cities[i+2, k])) > point[1]) {
                  point[1] <- as.numeric(cities[i, k]) + as.numeric(cities[i+1, k]) + as.numeric(cities[i+2, k])
                  point[2] <- i
                }
            }
